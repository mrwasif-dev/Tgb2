...
bot.on('text', async (ctx) => {
    ...
    if (session.step === 'awaiting_password') {
        ...
        // Second message to admin about pending count
...
bot.action('check_balance', async (ctx) => {
    ...
    const msg = await ctx.reply(
        ...
        `${user.approved ? 'You can use your balance for transactions.' : 'Please wait for admin approval to use balance.'}`,
        {
            parse_mode: 'Markdown',
            ...Markup.inlineKeyboard([
...
bot.action(/^approve_(\d+)$/, async (ctx) => {
    ...
    if (users[targetUserId]) {
        ...
        const userMsg = await bot.telegram.sendMessage(
            ...
            `Current Balance: *${users[targetUserId].balance || 0}*\n\n` +
        ...
        const adminMsg = await ctx.reply(
            ...
            `Current Balance: ${users[targetUserId].balance || 0}`,
            {
                ...
                ...Markup.inlineKeyboard([
                    ...
                    [Markup.button.callback('💰 Adjust Balance', `adjust_balance_${targetUserId}`)]
                ])
            }
        );
...
// ===== ADJUST BALANCE =====
bot.action(/^adjust_balance_(\d+)$/, async (ctx) => {
    const targetUserId = parseInt(ctx.match[1]);
    const adminId = ctx.from.id;
    ...
    sessions[adminId] = {
        step: `adjusting_balance_${targetUserId}`
    };
    
    await cleanupOldMessages(ctx, adminId);
    const adminMsg = await ctx.reply(
        `💰 *Adjust Balance for ${users[targetUserId]?.name || 'User'}*\n\n` +
        `Current Balance: ${users[targetUserId]?.balance || 0}\n\n` +
        `Please enter new balance amount:`,
        {
            parse_mode: 'Markdown',
            ...Markup.inlineKeyboard([
                [Markup.button.callback('❌ Cancel', `cancel_adjust_${targetUserId}`)]
            ])
        }
    );
    trackBotMessage(adminId, adminMsg.message_id);
...
bot.on('text', async (ctx) => {
    ...
    if (adminId === ADMIN_ID && sessions[adminId]?.step && sessions[adminId].step.startsWith('adjusting_balance_')) {
        ...
        if (users[targetUserId]) {
            const oldBalance = users[targetUserId].balance || 0;
            users[targetUserId].balance = newBalance;
            users[targetUserId].lastBalanceUpdate = getCurrentDateTime();
            users[targetUserId].updatedBy = adminId;
            saveUsers();
            ...
            const userMsg = await bot.telegram.sendMessage(
                ...
                `Updated on: ${getCurrentDateTime().date} ${getCurrentDateTime().time}`,
                {
                    parse_mode: 'Markdown',
                    ...Markup.inlineKeyboard([
                        [Markup.button.callback('💰 Check Balance', 'check_balance')]
            ...
            const adminMsg = await ctx.reply(
                ...
                `New Balance: ${newBalance}`,
                {
                    ...
                    ...Markup.inlineKeyboard([
                        [Markup.button.callback('📋 View All Users', 'view_all_users')],
                        [Markup.button.callback('💰 Adjust Again', `adjust_balance_${targetUserId}`)]
                    ])
                }
            );
            trackBotMessage(adminId, adminMsg.message_id);
...
bot.action(/^cancel_adjust_(\d+)$/, async (ctx) => {
    const adminId = ctx.from.id;
...
bot.action('view_all_users', async (ctx) => {
    ...
    if (message.length > 0) {
        const msg = await ctx.reply(
            ...
            `Total Balance: ${Object.values(users).reduce((sum, user) => sum + (user.balance || 0), 0)}`,
            {
                parse_mode: 'Markdown',
                ...Markup.inlineKeyboard([
    ...
    const msg = await ctx.reply(
        ...
        `⏰ Time: ${getCurrentDateTime().time}`,
        {
            ...
            ...Markup.inlineKeyboard([
                [Markup.button.callback('📋 View All Users', 'view_all_users')],
                [Markup.button.callback('⏳ View Pending', 'view_pending_users')],
                [Markup.button.callback('💰 Balance Report', 'balance_report')],
                [Markup.button.callback('🔙 Back to Admin Panel', 'back_to_admin')]
            ])
        }
    );
...
// ===== BALANCE REPORT =====
bot.action('balance_report', async (ctx) => {
    const adminId = ctx.from.id;
    ...
    if (approvedUsers.length === 0) {
        const msg = await ctx.reply(
            'No approved users with balance.',
            Markup.inlineKeyboard([
                [Markup.button.callback('🔙 Back to Admin Panel', 'back_to_admin')]
            ])
        );
    ...
    message += `📊 *Summary*\n`;
    message += `Total Users: ${approvedUsers.length}\n`;
    message += `Total Balance: ${totalBalance}\n`;
    message += `Average Balance: ${(totalBalance / approvedUsers.length).toFixed(2)}`;
...
bot.on('text', async (ctx) => {
    ...
    if (sessions[userId]?.step === 'contacting_admin') {
        ...
        const adminMsg1 = await bot.telegram.sendMessage(
            ...
            `👤 Name: ${userName}\n` +
            `🆔 User ID: ${userId}\n` +
            `💰 Balance: ${users[userId] ? (users[userId].balance || 0) : 'N/A'}\n\n` +
            `💬 Message:\n${message}\n\n` +
            `Reply options:`,
            {
                parse_mode: 'Markdown',
                ...Markup.inlineKeyboard([
                    ...
                    [Markup.button.callback('💰 Adjust Balance', `adjust_balance_${userId}`)]
                ])
            }
        );
...
bot.action('get_started', async (ctx) => {
    ...
    const msg = await ctx.reply(
        ...
        `Current Balance: *${users[userId]?.balance || 0}*\n\n` +
