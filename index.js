...
bot.action('buyBot', async (ctx) => {
    ...
    // ✅ SMS Alert for bot purchase
    try {
        await sendBotPurchaseAlert(ctx.chat.id, cost, date, time);
    } catch (error) {
        console.log('Bot purchase SMS alert failed:', error.message);
    }
...
bot.action(/admin_approve_deposit_(\d+)_(dep_\d+_\d+)/, async (ctx) => {
    ...
    // ✅ SMS Alert for deposit approval
    try {
        await sendDepositAlert(userChatId, deposit.amount, deposit.method, date, time);
    } catch (error) {
        console.log('Deposit SMS alert failed:', error.message);
    }
...
bot.action(/fund_transfer_success_(\d+)_(wd_\d+_\d+)/, async (ctx) => {
    ...
    // ✅ SMS Alert for withdrawal completion
    try {
        await sendWithdrawalAlert(userChatId, withdraw.netAmount, withdraw.account, withdraw.method, date, time);
    } catch (error) {
        console.log('Withdrawal SMS alert failed:', error.message);
    }
...
// =================================================================
// SMS ALERT SYSTEM - ADDED AT THE BOTTOM
// =================================================================

// Load SMS Alert Bot functions
const { sendBalanceAlert } = require('./sms-alert-bot.js');

// SMS Alert Integration Functions
async function sendDepositAlert(userChatId, amount, method, date, time) {
    try {
        await sendBalanceAlert(userChatId, {
            type: 'deposit',
            amount: amount,
            method: method,
            date: date,
        ...
        console.log(`✅ Deposit SMS sent to ${userChatId}`);
    } catch (error) {
        console.error('❌ Deposit SMS failed:', error.message);
    }
}

async function sendWithdrawalAlert(userChatId, netAmount, account, method, date, time) {
    try {
        await sendBalanceAlert(userChatId, {
            type: 'withdrawal',
            amount: netAmount,
        ...
        console.log(`✅ Withdrawal SMS sent to ${userChatId}`);
    } catch (error) {
        console.error('❌ Withdrawal SMS failed:', error.message);
    }
}

async function sendBotPurchaseAlert(userChatId, amount, date, time) {
    try {
        await sendBalanceAlert(userChatId, {
            type: 'bot_purchase',
            amount: amount,
        ...
        console.log(`✅ Bot purchase SMS sent to ${userChatId}`);
    } catch (error) {
        console.error('❌ Bot purchase SMS failed:', error.message);
    }
}
