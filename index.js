...
bot.action('upgradePlanMenu', async (ctx) => {
    ...
    // Find available upgrades (plans with higher price)
...
bot.action('adminViewNewPlanRequests', async (ctx) => {
    ...
    pendingPlans.forEach((req, index) => {
        ...
        message += `⚡ Quick Actions:\n`;
        message += `• Approve: /approve_plan_${req.id}\n`;
        message += `• Reject: /reject_plan_${req.id}\n\n`;
    });
...
bot.action('adminViewUpgradeRequests', async (ctx) => {
    ...
    pendingUpgrades.forEach((req, index) => {
        ...
        message += `⚡ Quick Actions:\n`;
        message += `• Approve: /approve_upgrade_${req.id}\n`;
        message += `• Reject: /reject_upgrade_${req.id}\n\n`;
    });
...
// When admin rejects, buttons are removed immediately and reason is asked
...
bot.action(/admin_reject_deposit_(\d+)_(dep_\d+_\d+)/, async (ctx) => {
    ...
    try {
        ...
        // Ignore error if message doesn't exist
    }
...
// ======= FIXED APPROVAL FLOW =======
// When admin approves plan, buttons are removed and URL is asked
...
bot.action(/admin_approve_plan_(\d+)_(plan_\d+_\d+)/, async (ctx) => {
    ...
    // Send first notification to user
    try {
        const userSession = sessions[userChatId];
        if (userSession && userSession.usernameKey) {
            const user = users[userSession.usernameKey];
            const pendingPlan = user.pendingPlans?.find(p => p.id === planId);
...
bot.action(/admin_approve_upgrade_(\d+)_(upgrade_\d+_\d+)/, async (ctx) => {
    ...
    try {
        ...
        if (userSession && userSession.usernameKey) {
            const user = users[userSession.usernameKey];
            const pendingUpgrade = user.pendingUpgrades?.find(u => u.id === upgradeId);
            
            if (pendingUpgrade) {
                await bot.telegram.sendMessage(
                    userChatId,
                    `✅ Upgrade Request Approved! ✅\n\n🎉 Great news! Your upgrade to ${pendingUpgrade.toPlan} has been approved.\n\n📋 Upgrade Details:\n🔄 From: ${pendingUpgrade.fromPlan}\n🎯 To: ${pendingUpgrade.toPlan}\n💰 Upgrade Cost: ${pendingUpgrade.cost} PKR\n\n🔄 Current Status: Upgrade Activation in Progress ⏳\n\n💡 You will receive new WhatsApp link shortly.`
                );
            }
        }
    } catch (error) {
...
// This includes viewTransactions, logOut, backToMenu, admin functions, etc.
